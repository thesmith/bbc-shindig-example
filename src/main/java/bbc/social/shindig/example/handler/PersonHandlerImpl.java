...
public class PersonHandlerImpl extends PersonHandler {
	...
	@Inject
	public PersonHandlerImpl(PersonService personService, RelationshipService relationshipService,
	...
	/**
	 * Create a new friendship between 2 users
	 * POST - {id: 'friendId'}
  ...
  public Future<?> createFriends(SocialRequestItem request) throws ProtocolException {
    ...
    if (person == null || person.getId() == null || person.getId().length() < 1) {
      throw new IllegalArgumentException("Cannot create relationship without a person to befriend");
    }
    UserId userId = userIds.iterator().next();
    this.relationshipService.createRelationship(userId.getUserId(request.getToken()), person.getId());
